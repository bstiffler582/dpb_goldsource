...
void DLLEXPORT CAM_Think( void )
{
	...
	else
	{
		...
		if( camAngles[2] < cam_idealdist->value )
			camAngles[2]=min(camAngles[2]+gHUD.m_flTimeDelta*120.0,cam_idealdist->value);
		else if(camAngles[2]>cam_idealdist->value) 
			camAngles[2]=max(camAngles[2]-gHUD.m_flTimeDelta*120.0,cam_idealdist->value);
	}
#ifdef LATER
	if( cam_contain->value )
	{
