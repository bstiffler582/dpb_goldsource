...
*
****/
#define  PM_SHARED_C /* Valve's stupid struct redefinitions */
...
#define TRANSITION_RATE 124.0
float endpoint;
void PM_ViewHeight()
{
	if(pmove->view_ofs[2]>endpoint) {
		pmove->view_ofs[2]-=pmove->frametime*TRANSITION_RATE;
		if(pmove->view_ofs[2]<endpoint)
			pmove->view_ofs[2]=endpoint;
	} else if(pmove->view_ofs[2]<endpoint) {
		pmove->view_ofs[2]+=pmove->frametime*TRANSITION_RATE;
		if(pmove->view_ofs[2]>endpoint)
			pmove->view_ofs[2]=endpoint;
	}
...
void PM_UnProne( void )
{
	...
	if ( !trace.startsolid )
	{
		...
		if ( trace.startsolid )
		{
			// See if we are stuck?  If so, stay proned with the prone hull until we have a clear spot
			//Con_Printf( "unstick got stuck\n" );
			pmove->usehull = 3;
			endpoint=VEC_PRONE_VIEW;
			return;
		}
		pmove->iuser4 &= ~(U_PRONING|U_INPRONE);
		pmove->view_ofs[2] -= f;
		endpoint=VEC_VIEW;
		pmove->flDuckTime = 0;
...
void PM_Prone( void )
{
	...
	if ( pmove->cmd.buttons & IN_PRONE )
	{
		pmove->oldbuttons |= IN_PRONE;
	}
	else
	{
		pmove->oldbuttons &= ~IN_PRONE;
	...
	// Prevent proning if the iuser3 variable is set
	if ( pmove->iuser3 || pmove->dead )
	{
		// Try to unprone
		if ( pmove->iuser4 & U_PRONING )
		{
			PM_UnProne();
		}
		return;
	...
	if ( pmove->iuser4 & U_PRONING )
	{
		pmove->cmd.forwardmove *= 0.25;
		pmove->cmd.sidemove    *= 0.25;
		pmove->cmd.upmove      *= 0.25;
	}

	if ( ( pmove->cmd.buttons & IN_PRONE ) || ( pmove->iuser4& U_INPRONE ) || ( pmove->iuser4 & U_PRONING ) )
	{
		fMore = ((-32.0)-pmove->player_mins[pmove->usehull][2]);
		if (!(pmove->iuser4&(U_PRONING|U_INPRONE))
			 && pmove->onground!=-1)
		{
			pmove->usehull = 3;
			// HACKHACK - Fudge for collision bug - no time to fix this properly
			pmove->origin[2] -= fMore;
			// See if we are stuck?
			if(pmove->PM_TestPlayerPosition(pmove->origin,NULL)!=-1) {
				proneFraction=pmove->origin[2];
				PM_FixPlayerCrouchStuck( STUCK_MOVEUP );
				if(pmove->origin[2]==proneFraction) {
					pmove->usehull = 0;
					pmove->origin[2]+=fMore;
					return;
				}
			...
			pmove->iuser4 |= U_INPRONE;
			pmove->view_ofs[2]+=fMore;
		...
		if ( pmove->iuser4& U_INPRONE )
		{
			// Finish ducking immediately if duck time is over or not on ground
			if ( (float)pmove->flDuckTime / 1000.0 <= ( 1.0 - TIME_TO_DUCK ) ||
				pmove->onground==-1 ) 
			{
				pmove->iuser4 = (pmove->iuser4&~U_INPRONE)|U_PRONING;
			}
			/*else
			{

				// Calc parametric time
				proneFraction = PM_SplineFraction( time, (1.0/TIME_TO_DUCK) );
				pmove->view_ofs[2] = ((VEC_PRONE_VIEW ) * proneFraction) + ((VEC_VIEW+fMore) * (1-proneFraction));
			}*/
		}
		endpoint=VEC_PRONE_VIEW;
	}
...
void PM_UnDuck( void )
{
	...
	float f;
	VectorCopy( pmove->origin, newOrigin );
	f=( pmove->player_mins[1][2] - pmove->player_mins[0][2] );
	newOrigin[2] += f;
	trace = pmove->PM_PlayerTrace( newOrigin, newOrigin, PM_NORMAL, -1 );
...
void PM_Duck( void )
{
	...
	if ( pmove->flags & FL_DUCKING )
	{
		pmove->cmd.forwardmove *= 0.33;
		pmove->cmd.sidemove    *= 0.33;
		pmove->cmd.upmove      *= 0.33;
	}

	if ( ( pmove->cmd.buttons & IN_DUCK ) || ( pmove->bInDuck ) || ( pmove->flags & FL_DUCKING ) )
	{
		fMore=pmove->player_mins[1][2]-pmove->player_mins[pmove->usehull][2];
		if ( pmove->onground!=-1 &&
				(nButtonPressed & IN_DUCK ) && !( pmove->flags & FL_DUCKING ) )
		{
			// Use 1 second so super long jump will work
			pmove->flDuckTime = 1000;
			pmove->bInDuck    = true;
		...
		endpoint=VEC_DUCK_VIEW;//+pmove->player_mins[pmove->usehull][2]-pmove->player_mins[1][2];
//		pmove->Con_Printf("WOWOWOW %f %f\n",endpoint,endpoint-pmove->player_mins[pmove->usehull][2]);
	}
}
...
void PM_Jump (void)
{
	...
	if ( pmove->oldbuttons & IN_JUMP|| pmove->bInDuck||pmove->flags&FL_DUCKING
			|| pmove->iuser4&U_PRONING||pmove->iuser4&U_INPRONE )
		return;
...
void PM_PlayerMove ( qboolean server )
{
	...
	/* VALVE IS RETARDED AND RESETS HULLS IF NOT DUCKING */
	if(pmove->iuser4&U_PRONING||pmove->iuser4&U_INPRONE) pmove->usehull=3;
	if(pmove->flags&FL_DUCKING||pmove->bInDuck) pmove->usehull=1;
  ...
  if(!pmove->dead)
		PM_UpdateStepSound();
	endpoint=VEC_VIEW;
	if((pmove->flags&FL_DUCKING||pmove->bInDuck)&&!(pmove->cmd.buttons&IN_DUCK))
		PM_UnDuck();
	if(pmove->iuser4&(U_INPRONE|U_PRONING)&&!(pmove->cmd.buttons&IN_PRONE))
		PM_UnProne();
	PM_Duck();
...
/*	if(pmove->physents[0].model[0].hulls[3].clip_maxs[2]!=30.0
		...
		for(i=0;i<4;i++) {  
			...
			VectorCopy(player_maxs[i],pmove->player_maxs[i]);
			for(j=0;j<1024;j++) {
				VectorCopy(player_mins[i],
						pmove->physents[0].model[j].hulls[world_shuffle[i]].clip_mins);
				VectorCopy(player_maxs[i],
